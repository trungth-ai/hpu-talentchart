...
from app.data.questions import DISC_QUESTIONS, PERSONALITY_CATEGORIES, DISC_DESCRIPTIONS, DISC_POSITION_PROFILES

def calculate_disc(disc_answers: Dict[str, Dict[str, int]]) -> Dict:
...
def generate_analysis(disc_result: Dict, personality_scores: Dict[str, float], position: str) -> Dict:
    """Generate comprehensive analysis for candidate."""
    ...
    # Recommendation
    overall_score = round((avg_personality * 0.5 + fit_score * 0.3 + min(disc_result["disc_scores"].get(primary, 0), 100) * 0.2))
...
def generate_ai_interview_suggestions(disc_result: Dict, personality_scores: Dict, position: str) -> Dict:
    """Generate AI-powered interview question suggestions based on DISC profile."""
    primary = disc_result["disc_primary"]
    secondary = disc_result["disc_secondary"]
    profile = disc_result["disc_profile"]
    ...
    disc_questions_map = {
        "D": {
            "focus": ["Kiểm tra khả năng lắng nghe và làm việc nhóm", "Đánh giá sự kiên nhẫn với quy trình"],
            "questions": [
                "Kể về lần gần nhất bạn phải nhường nhịn trong một quyết định nhóm. Bạn cảm thấy thế nào?",
            ...
            "red_flags": ["Không thể nêu ví dụ về lắng nghe người khác", "Luôn muốn kiểm soát mọi thứ"],
        },
        "I": {
            "focus": ["Kiểm tra tính kiên trì và hoàn thành công việc chi tiết", "Đánh giá khả năng làm việc độc lập"],
            "questions": [
                "Mô tả một dự án yêu cầu bạn làm việc một mình trong thời gian dài. Bạn duy trì động lực như thế nào?",
            ...
            "red_flags": ["Không có ví dụ cụ thể về hoàn thành công việc chi tiết", "Né tránh câu hỏi về kỷ luật"],
        },
        "S": {
            "focus": ["Kiểm tra khả năng ra quyết định dưới áp lực", "Đánh giá tính chủ động và sáng tạo"],
            "questions": [
                "Kể về lần bạn phải đưa ra quyết định nhanh mà không có đủ thông tin. Kết quả ra sao?",
        ...
        "C": {
            "focus": ["Kiểm tra khả năng giao tiếp và linh hoạt", "Đánh giá cách xử lý khi không có đủ dữ liệu"],
            "questions": [
                "Khi phải thuyết trình một chủ đề phức tạp cho người không chuyên, bạn làm thế nào?",
            ...
            "red_flags": ["Quá cứng nhắc, không linh hoạt", "Không thể ra quyết định khi thiếu dữ liệu"],
        },
    }
    ...
    personality_questions = {
        "integrity": "Kể về tình huống bạn phải chọn giữa lợi ích cá nhân và sự trung thực. Bạn đã chọn gì?",
        ...
        "discipline": "Bạn quản lý thời gian và deadline như thế nào khi có nhiều công việc cùng lúc?",
        "adaptability": "Kể về lần bạn phải thay đổi phương pháp làm việc giữa chừng. Kết quả ra sao?",
        "leadership": "Mô tả tình huống bạn phải đưa team đi đúng hướng khi mọi thứ đang hỗn loạn.",
