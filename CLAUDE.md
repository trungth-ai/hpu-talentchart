...
- Pipeline ứng viên có đúng 7 trạng thái, chỉ đi tuần tự: `NEW → SCREENING → TEST_SENT → TEST_DONE → INTERVIEW → DECISION → HIRED/REJECTED`. Không cho phép nhảy cóc trạng thái từ API.
